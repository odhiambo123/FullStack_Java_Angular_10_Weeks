...
mysql> `insert into address (city, country, cust_id) values ('NY', 'USA', 3);`
ERROR 1452 (23000): Cannot add or update a child row: a foreign key constraint fails (`demo`.`address`, CONSTRAINT `address_ibfk_1` FOREIGN KEY (`cust_id`) REFERENCES `customer` (`cust_id`))
mysql>



# JDBC
JDBC stands for Java Database Connctivity
JDBC is an Java API to connect and execute the query with the database
...
A list of popular intterface
- Driver interface
- Connection interface
- Statement Interface
- PrepareStatement interface
- ResultSet interfacve
...
3. retrive the reesult recieved from the database
